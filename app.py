...
def load_models():
    ...
    try:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            "distilbert-finetuned-stackexchange-multi-label-20250407T055524Z-001/distilbert-finetuned-stackexchange-multi-label"
        )
...
def model_test():
    test_question = "How to debug Python code?"
    try:
        # Verify components
        if not all([model, tokenizer, multilabel]):
            raise Exception("One or more components not loaded")
            
        # Test prediction
        inputs = tokenizer(test_question, return_tensors="pt", padding=True, truncation=True)
        with torch.no_grad():
            outputs = model(**inputs)
            logits = outputs.logits
            probs = torch.sigmoid(logits)
        ...
        return jsonify({
            ...
            'components': {
                ...
                'multilabel_binarizer': multilabel is not None
            }
        })
    except Exception as e:
        return jsonify({
...
def model_status():
    return jsonify({
        ...
        'multilabel_loaded': bool(multilabel),
        'status': 'ready' if all([model, tokenizer, multilabel]) else 'incomplete'
    })
